...
也可以打开 [最新版 Release 页](https://github.com/luckduckone/telecom-assistant-releases/releases/latest) 下载带版本号的原始文件(`电信助手-vX.X.X-release.apk` / `电信助手-vX.X.X-未签名.ipa`)。

## 📱 安装说明

### Android

1. 下载 `latest.apk`(或任意版本的 `电信助手-vX.X.X-release.apk`);
2. 允许「安装未知来源应用」后直接安装。
...
2. 用自签工具重签后安装,任选其一:
   ...
   - 有证书的用户:下载该版本附件中的 `打包签名IPA.sh`、`签名校验.sh`,按脚本内注释全量重签(**小组件必须全量签名,否则闪退**)。
